...
class CalendarScreenState extends State<CalendarScreen> {
  ...
  Future<void> addRandomNapEvent(DateTime selectedDay) async {
    ...
    DateTime startTime = startTimeLimit.add(Duration(minutes: _random.nextInt((endTimeLimit.difference(startTimeLimit)).inMinutes)));

    int duration = _random.nextInt(45) + 15; // Random duration in minutes (15-60)
    DateTime endTime = startTime.add(Duration(minutes: duration));

    // Check if the generated time is within the limit
    if (startTime.isAfter(startTimeLimit) && endTime.isBefore(endTimeLimit)) {
        ...
        showDialog(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content: const Text('An error occurred: $e'),
              actions: [
                TextButton(
                  onPressed: () {
                    Navigator.pop(context);
